    ...
    <div class="row">
        ...
        <div ref="jobTable">
            <a-table :columns="tableHeader" rowKey="id" :data-source="tableData" :pagination="pageConfig" :scroll="{y: 'calc(100vh - 200px)',x: 'calc(100vw - 10W)'}">
                ...
                <div slot="job_title" slot-scope="text">
            ...
            <a-modal v-model="visible" :footer="null" :closable="false" :wrapClassName="'img-modal'" :getContainer="() => $refs.jobTable">
                ...
                <div class="modal-content">
                    <div class="form">
                        <template v-if="modalData.type=='pass'">
                            <a-row :gutter="[0, 10]">确认审核通过？</a-row>
                        </template>
...
import { api_content_management_job_list,api_content_management_job_pass,api_content_management_job_refuse, api_content_management_job_down } from '@/http/content-management/job/list';
...
export default {
    ...
    methods: {
        async init_data(){
            const _res = await api_content_management_job_list(this.formData);
            if (_res.result){
        ...
        async handle_modal_confirm(modalData){
            switch (modalData.type){
                case 'pass':
                    var _params = { id: this.modalData.id };
                    var _res = await api_content_management_job_pass(_params);
                    break;
                case 'refuse':
                    _params = {  id: this.modalData.id, reason_id: this.modalData.reason_id };
                    _res = await api_content_management_job_refuse(_params);
                    break;
                case 'down':
                    _params = {  id: this.modalData.id, audit_reason: this.modalData.reason};
                    _res = await api_content_management_job_down(_params);
                    break;
            }
            if (_res.result){
